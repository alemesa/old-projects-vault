...
const searchQuery = `https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&query=Gladiator`;
...
new Vue({
  ...
  data: {
    ...
    imdbID: '',
    url:
      'https://www.utopolis.lu/bundles/utopoliscommon/images/movies/movie-placeholder.j' +
      'pg',
    imdbID: 'http://www.imdb.com/title/',
    search: false,
    movie: false
  },
  methods: {
    ...
    findData: function() {
      ...
      if (this.title !== '') {
        ...
        for (var i = 0; i < decompose.length; i++) {
          if (i < decompose.length - 1) searchterm += decompose[i] + '+';
          else searchterm += decompose[i];
        }
        ...
        axios
          ...
          .then(function(response) {
            ...
            if (response.data.results[0]) {
              ...
              axios
                .get(
                  `https://api.themoviedb.org/3/movie/${response.data.results[0]
                    .id}?api_key=0ceedd539b0a1efa834d0c7318eb6355`
                )
...
*/
                ...
                app.country += element.name + " | ";
                  ...
                  if (response2.data.production_companies[1].name) {
                    app.production +=
                      ' & ' + response2.data.production_companies[1].name;
                  }

                  app.imdbID =
                    'http://www.imdb.com/title/' +
                    response2.data.imdb_id +
                    '/?ref_=nv_sr_1';
                })
